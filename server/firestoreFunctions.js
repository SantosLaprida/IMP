...
export const restoreI_Players = async (players) => {
  const collectionRef = collection(firestore, 'I_Players');

  // Clean the I_Players collection
  ...
  for (const player of players) {
    const docRef = await addDoc(collectionRef, {
      name: player.name,
      rank: player.rank,
      // id_player is not set here because it's auto-generated by Firestore
    });
    // Now, update the document to include the id_player field with the auto-generated ID
    await setDoc(docRef, { ...player, id_player: docRef.id }, { merge: true });
  }
};
